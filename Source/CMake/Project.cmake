...
# MOD_DLL_ONLY: only enables build if DLL is enabled
# MOD_SWIG: Enable Swig generation for this target
#
# Public defines:
...
# MOD_SWIG_SOURCES: files where all the SWIG sources are includes
...
macro(generate_project)
    ...
    if (MOD_ONLY_DLL)
        set(MOD_NO_DLL 0) # cannot disable DLL if it's a DLL only module
    endif()
    ...
    foreach (SRC_PATH IN ITEMS ${MOD_SRC_PATHS})
        ...
        if (MOD_SWIG)
            file(GLOB SWIG_SRCS "${SRC_PATH}/*.i")
            list(APPEND SRCS ${SWIG_SRCS})
            list(APPEND MOD_SWIG_SOURCES ${SWIG_SRCS})
        endif()
    ...
    if (NOT MOD_ONLY_DLL)
        ...
        if (DEFINED MOD_SWIG)
            SwigTarget(${MOD_NAME} ${MOD_SWIG_SOURCES})
        endif()
    endif()
    ...
    if (GB3_ENABLE_DLL AND NOT MOD_NO_DLL) # for DLL modules
        ...
        if (DEFINED MOD_SWIG)
            SwigTarget(${MOD_NAME}DLL ${MOD_SWIG_SOURCES})
        endif()
    endif()
endmacro()
